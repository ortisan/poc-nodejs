import { CauseDto, ErrorDto } from '@core/shared/error/dto/error.dto';
import {
  BadRequestError,
  NotFoundError,
  GenericError,
...
} from '@core/shared/error/errors';
import { ILocaleService } from '@core/shared/i18n/locale.service.contract';
...
abstract class AbstractBaseExceptionFilter implements ExceptionFilter {
  ...
  catchError(error: GenericError, host: ArgumentsHost, statusCode: number) {
    ...
    const errorDto = this.translateError(
      GenericError.fromError(error),
      language,
    );
  ...
  translateError(exception: GenericError, language: string): ErrorDto {
    const causes = exception.causes?.map((cause) => {
      return new CauseDto({
        code: cause.code,
        message: this.localeService.getMessage(
          language,
          cause.code,
          cause.arguments,
        ),
